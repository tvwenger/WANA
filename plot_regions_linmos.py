"""
plot_regions_linmos.py - WANA image plotting script (with regions)
...
Trey V. Wenger September 2019 - V2.0
    Update for WISP V2.0 support (stokes image names and mosaics)
...
def main(field,spws,stokes='I',
         ...
         legend_loc='upper right', wise_legend_loc='lower right',
         legend_fontsize=6):
    """
    ...
    for spw in spws.split(','):
        ...
        #
        pbcorr_data = image_hdu.data[0,0]
        image_data = image_hdu.data[0,0]*pb_hdu.data[0,0]
        res_data = residual_hdu.data[0,0]*pb_hdu.data[0,0]
        #
        # Mask outside 5% weights
        #
        clip = (pb_hdu.data[0,0] < 0.05)+np.isnan(pb_hdu.data[0,0])
        pbcorr_data[clip] = np.nan
        image_data[clip] = np.nan
        res_data[clip] = np.nan
        #
        # Compute RMS from uncorrected residuals (MAD)
        # The image noise can vary significantly across the mosaic.
        #
        sigma = 1.4826*np.nanmedian(np.abs(res_data-np.nanmedian(res_data)))
        #
        # Generate figure
        #
        plt.ioff()
        ...
        cax = ax.imshow(pbcorr_data,origin='lower',
                        interpolation='none',cmap='binary')
        # contours are non-PB corrected
        if sigma > 0.:
            con = ax.contour(image_data,origin='lower',
                             levels=levels*sigma,colors='k',linewidths=0.2)
        xlen,ylen = pbcorr_data.shape
        ...
        if wisefile is not None:
            ...
            patches = [ell]
            wise_legend = plt.legend(handles=patches,loc=wise_legend_loc,fontsize=6,
            ...
            if len(peak_regions) == 0:
                ...
                for dat in wisedata:
                    ...
                    #
                    clip = image_data > levels[0]*sigma
                    #
        ...
        #
        if len(peak_regions) > 0:
            region_legend = plt.legend(loc=legend_loc,fontsize=legend_fontsize)
